...
The bandit early stopping policy `BanditPolicy` that terminates runs where the primary metric is not within the specified slack factor/slack amount compared to the best performing run was used to improve computational efficiency. The early policy termination was set to a `slack factor of 0.1` and `evaluation interval of 1` so that performance is evaluated at the end of each child run.
